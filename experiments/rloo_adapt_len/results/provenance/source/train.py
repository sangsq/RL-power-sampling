...
from common import HERE, ROOT, append, log, rows, sha, spec, write
...
def main():
    ...
    name = 'calibration' if args.calibrate else 'alpha2_adapt_len'
    out = HERE/'results'/name
    out.mkdir(exist_ok=False)
    started = time.monotonic()
    deadline = started+(550 if args.calibrate else s['internal_deadline_seconds'])
    ...
    config = {**s, 'calibration': args.calibrate, 'settings_sha256': sha(HERE/'settings.json'),
